...
## Live Workshop Testing Rule

Use this sequence for every implementation:

1. Run `./gradlew compileJava`.
2. Fix compile errors and rerun `./gradlew compileJava` until it passes.
3. Run the test for the class or feature just changed.
4. Run the relevant slice test (`@WebMvcTest`, `@DataJpaTest`, `@SpringBootTest`, or Vaadin UI smoke test).
5. Run `./gradlew check`.

Do not add containerized test infrastructure to the live workshop path. Use H2, mocked ports, or existing local fixtures.
...
## Integration Testing With Local Test Profile
