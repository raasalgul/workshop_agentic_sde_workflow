...
## Vaadin + Spring Security Notes

- Prefer Vaadin's Spring Security integration when the project uses server-side Vaadin Flow.
- Route access should be explicit with annotations such as `@AnonymousAllowed`, `@PermitAll`, `@RolesAllowed`, or the project's established access checker.
- Keep login/logout routes stable and demo-friendly; verify navigation after authentication, not only the raw security filter chain.
- Permit Vaadin internal requests according to the Vaadin security helper used by the project version.
